...
class ViPTTrack(BaseTracker):
    ...
    def initialize(self, image, info: dict):
        ...
        self.state_clean = info['init_bbox']
    ...
    def track_adv_ours(self, image, search_tensor, search_x_tensor, vi_adv, ir_adv, gt, info: dict = None ):
        ...
        if self.debug == 1:
            # search track debug&vis
            pred_box_search = (pred_boxes.mean(dim=0) * self.params.search_size).to(torch.float64)
            x1, y1, w, h = pred_box_search
            image_adv = tensor_to_cv2(img_cat_search[0,:3,:,:])
            image = tensor_to_cv2(img_cat_search_clean[0,:3,:,:])

            image_BGR_adv = cv2.cvtColor(image_adv, cv2.COLOR_RGB2BGR)
            image_BGR = cv2.cvtColor(image, cv2.COLOR_RGB2BGR)

            cv2.rectangle(image_BGR, (int(x1- 0.5*w), int(y1 - 0.5*h)), (int(x1 + 0.5*w), int(y1 + 0.5*h)), color=(0, 0, 255), thickness=2)
            cv2.rectangle(image_BGR_adv, (int(x1- 0.5*w), int(y1 - 0.5*h)), (int(x1 + 0.5*w), int(y1 + 0.5*h)), color=(0, 0, 255), thickness=2)

            cv2.imwrite(f'./img_test/search_attack.jpg',image_BGR_adv)
            cv2.imwrite('./img_test/search.jpg',image_BGR)
