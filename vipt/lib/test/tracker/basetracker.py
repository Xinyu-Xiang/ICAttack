...
class BaseTracker:
    ...
    def transform_bbox_to_crop(self, box_in, resize_factor, device, box_extract=None, crop_type='template'):
        ...
        if crop_type == 'template':
            crop_sz = torch.Tensor([self.params.template_size, self.params.template_size]).to(device)
        elif crop_type == 'search':
            crop_sz = torch.Tensor([self.params.search_size, self.params.search_size]).to(device)
        else:
            raise NotImplementedError

        box_in = torch.tensor(box_in).to(device)
        if box_extract is None:
            box_extract = box_in.to(device)
        else:
            box_extract = torch.tensor(box_extract).to(device)
