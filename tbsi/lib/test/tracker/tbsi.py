...
from tbsi.lib.models.tbsi_track import build_tbsi_track
from tbsi.lib.test.tracker.basetracker import BaseTracker
import torch

from tbsi.lib.test.tracker.vis_utils import gen_visualization
from tbsi.lib.test.utils.hann import hann2d
from tbsi.lib.train.data.processing_utils import sample_target
# for debug
import cv2
import os

from tbsi.lib.test.tracker.data_utils import Preprocessor
from tbsi.lib.utils.box_ops import clip_box
from tbsi.lib.utils.ce_utils import generate_mask_cond
...
class TBSITrack(BaseTracker):
    ...
    def track_adv_ours(self, image, search_tensor, search_x_tensor, vi_adv, ir_adv, gt, info: dict = None ):
        ...
        image concat
        vi,ir [1,3,256,256]
        vi_adv,ir_adv [1,3,256,256](0-255)
        ...
        # with torch.no_grad():
        x_dict = img_cat_search_norm
        # merge the template and the search
        # run the transformer
        out_dict = self.network.forward(
            template=[self.z_dict1.tensors[:,:3,:,:],self.z_dict1.tensors[:,3:,:,:]], search=[x_dict[:,:3,:,:], x_dict[:,3:,:,:]], ce_template_mask=self.box_mask_z)
        out_dict_clean = self.network.forward(
            template=[self.z_dict1.tensors[:,:3,:,:],self.z_dict1.tensors[:,3:,:,:]], search=[img_cat_search_clean_norm[:,:3,:,:], img_cat_search_clean_norm[:,3:,:,:]], ce_template_mask=self.box_mask_z)
        ...
        pred_boxes = pred_boxes.view(-1, 4)
        # Baseline: Take the mean of all pred boxes as the final result
        crop_z = math.ceil(math.sqrt(self.state[2] * self.state[3]) * self.params.search_factor)
