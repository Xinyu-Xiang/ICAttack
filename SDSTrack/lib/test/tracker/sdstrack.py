...
class SDSTrack(BaseTracker):
    ...
    def track_adv_ours(self, image, search_tensor, search_x_tensor, vi_adv, ir_adv, gt, info: dict = None ):
        ...
        # with torch.no_grad():
            # merge the template and the search
            # run the transformer
        out_dict = self.network.forward(template=self.z_tensor, search=img_cat_search_norm, ce_template_mask=self.box_mask_z, infer=True) 
        out_dict_clean = self.network.forward(template=self.z_tensor, search=img_cat_search_clean_norm, ce_template_mask=self.box_mask_z,infer=True)
